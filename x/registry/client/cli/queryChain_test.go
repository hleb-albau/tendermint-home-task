...
func networkWithChainObjects(t *testing.T, n int) (*network.Network, []*types.Chain) {
	...
	for i := 0; i < n; i++ {
		state.ChainList = append(state.ChainList, &types.Chain{Owner: "ANY", ChainID: strconv.Itoa(i)})
	}
	state.ChainCount = uint64(n)
...
func TestShowChain(t *testing.T) {
	net, objs := networkWithChainObjects(t, 2)
...
func TestListChain(t *testing.T) {
	net, objs := networkWithChainObjects(t, 5)
